...
BYTE DetermThGonzalez(int* H)
{
	BYTE High, Low;
	BYTE Th;		// 임계값
	BYTE NewTh;		// 새로운 임계값 저장될 변수
	BYTE ep = 3;	// 입실론 : 오차
	int G1 = 0, cnt1=0, mu1,	// 임계값 T 초과 화소값들의 누적합(G1), 개수(cnt1), 평균(mu1) 
		G2 = 0, cnt2=0, mu2;		// 임계값 T 이하 화소값들의 누적합(G2), 개수(cnt2), 평균(mu2) 
...
{
	Ty *= -1; //BMP 상하 뒤집힌 것 고려
	for (int i = 0; i < H; i++)
	{
		for (int j = 0; j < W; j++)
		{
			if ((i + Ty) < H && (i + Ty) >= 0 && (j + Tx) < W && (j + Tx) >= 0)	// 범위 내 예외처리
...
void Rotation(BYTE* Image, BYTE* Output, int W, int H, int Angle)
{
	...
	//		if (tmpY < H && tmpY >= 0 && tmpX < W && tmpX >= 0)	// 범위 내 예외처리
	//			Output[tmpY * W + tmpX] = Image[i * W + j];
	//	}
	//}
...
// HW5 : 영상을 순방향 사상으로 가로 및 세로 3배 확대하고, 비어있는 영역을 bilinear interpolation 수행하여 채우는 프로그램을 구현
void ScalingForward3_BilinearInterpolation(BYTE* Image, BYTE* Output, int W, int H)
{
	double SF_X = 3.0, SF_Y = 3.0;
	int tmpX, tmpY;
	// 영상을 순방향 사상으로 가로 및 세로 3배 확대
	for (int i = 0; i < H; i++)
	{
		for (int j = 0; j < W; j++)
		{
	...
	for (int i = 0; i < H; i++) // 가로 방향 선형 보간
	{
		for (int j = 0; j < W; j+=3)
		{
			if (i % 3 == 1)			Output[j * W + i] = (int)(Output[j * W + i - 1] * 2.0 / 3) + (int)(Output[j * W + i + 2] * 1.0 / 3);
			else if (i % 3 == 2)	Output[j * W + i] = (int)(Output[j * W + i - 2] * 1.0 / 3) + (int)(Output[j * W + i + 1] * 2.0 / 3);
			else;					// Scaling 하며 원본 영상에서 가져온 값이 들어 있음
		}
	}

	for (int i = 0; i < H; i++) // 세로 방향 선형 보간
	{
		for (int j = 0; j < W; j++)
		{
			if (j % 3 == 1)			Output[j * W + i] = (int)(Output[j * W + i - 256] * 2.0 / 3) + (int)(Output[j * W + i + 512] * 1.0 / 3);
			else if (j % 3 == 2)	Output[j * W + i] = (int)(Output[j * W + i - 512] * 1.0 / 3) + (int)(Output[j * W + i + 256] * 2.0 / 3);
			else;					// Scaling 하며 원본 영상에서 가져온 값이 들어 있음
		}
	}
}
...
int main()
{
	...
	//Th = DetermThGonzalez(Histo); // 임계값 지정(Gonzalez, Woods 방법)
	...
	/* Median filtering */
	//int Length = 9;										// 마스크 한 변의 길이(5, 7, 9)
	...
	//Binarization(Image, Temp2, W, H, 30);			// 라벨링 위한 이진화 처리
	...
	// HW5 
	//ScalingForward3_BilinearInterpolation(Image, Output, W, H);	
